<template>
	<view>
		<view class="content" @touchstart="hideDrawer">
			<scroll-view class="msg-list" scroll-y="true" scroll-anchoring="true" :scroll-with-animation="scrollAnimation" :scroll-top="scrollTop"
		...
		<view class="input-box" :class="popupLayerClass" @touchmove.stop.prevent="discard">
			...
			<view class="textbox">
				...
				<view class="text-mode" :class="isVoice?'hidden':''">
					<view class="box">
						<textarea cursor-spacing="100" auto-height="true" v-model="textMsg" @focus="textareaFocus" />
						</view>
	...
	export default {
		...
		onLoad(option) {
			...
			uni.onSocketMessage((res) => {
			  ...
			  if(JSON.parse(res.data).myuid != this.myuid){
				  console.log("收到数据")
				  this.screenMsg(JSON.parse(res.data))
			  }
			});
		...
		methods:{
								...
								success: (image)=>{
									...
									uni.uploadFile({
										...
										success: (uploadFileRes) => {
											console.log("上传图片");
											let msg = {url:JSON.parse(uploadFileRes.data).url,w:image.width,h:image.height};
			...
			sendMsg(content,type){
				...
				lastid++;
				let msg = {type:'user',myuid:this.myuid,msg:{id:lastid,time:nowDate.getHours()+":"+nowDate.getMinutes(),type:type,userinfo:{uid:this.myuid,username:"老婆",face:"/static/img/girl.jpg"},content:content}}
				// 发送消息
